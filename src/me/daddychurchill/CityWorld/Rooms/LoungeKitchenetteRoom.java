...
public class LoungeKitchenetteRoom extends LoungeRoom {
	...
	public void drawFixture(CityWorldGenerator generator, RealBlocks chunk, Odds odds, int floor, int x, int y, int z,
		...
		case NORTH:
			...
			chunk.setBlock(x + 1, y, z + 2, Material.PISTON, BlockFace.UP);
			if (odds.flipCoin())
		...
		case WEST:
			...
			chunk.setCauldron(x + 1, y, z + 1, odds);
			chunk.setBlock(x + 2, y, z + 1, Material.PISTON, BlockFace.UP);
			if (odds.flipCoin())
	...
	private void placeStuff(CityWorldGenerator generator, RealBlocks chunk, Odds odds, int x, int y, int z,
			BlockFace facing) {
		if (generator.getSettings().treasuresInBuildings && odds.playOdds(generator.getSettings().oddsOfTreasureInBuildings))
